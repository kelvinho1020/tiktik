      ...
      <div className="flex gap-3 flex-wrap">
        {topics.map(item => (
          <Link href={`/?topic=${item.name}`} key={item.name}>
            <div className={`${topic === item.name ? activeTopicStyle : topicStyle} xl:border-2 hover:bg-primary px-3 py-2 rounded xl:rounded-full flex items-center gap-2 justify-center cursor-pointer`}>
